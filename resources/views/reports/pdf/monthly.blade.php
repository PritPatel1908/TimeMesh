...
<body>
    <div class="header">
        ...
        <h2>Monthly Report for {{ $extraData['month'] }}</h2>
    </div>

    <div class="filter-info">
        @if ($extraData['userCode'])
            <p>Filtered by User: {{ $extraData['userCode'] }}</p>
        @endif
        @if ($extraData['roomNumber'])
            <p>Filtered by Room: {{ $extraData['roomNumber'] }}</p>
        @endif
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="6" style="text-align: center;">No records found for the selected criteria.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
